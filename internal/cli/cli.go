...
func (c *CLI) processCandidate(candidate core.Candidate, entry *core.Entry) error {
	...
	// Execute action
	return c.executeAction(result.Action, entry)
}

func (c *CLI) executeAction(action rules.Action, entry *core.Entry) error {
	now := time.Now()
	...
	case core.IntentCommitment:
		...
		fmt.Printf("logged commitment to %s: %s (due %s)\n",
	...
	case core.IntentProgress:
		...
		fmt.Printf("logged progress on %s\n", action.Progress.ProjectID)
